...
type HashicorpAccountStoreConfig struct {
	Wallets          []VaultConfig `toml:",omitempty"`
...
func (c AccountConfig) AsAccount(urlPath string) *accounts.Account {
	return &accounts.Account{Address: common.HexToAddress(c.Address), URL: accounts.URL{Scheme: althashi.AcctScheme, Path: urlPath}}
}
